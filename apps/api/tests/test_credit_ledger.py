...
def test_reserve_release_roundtrip(session: Session):
    ...
    w.processing_minutes_available = 100
    session.add(w)
    session.commit()
    ...
    assert w2.processing_minutes_available == 70
    assert w2.processing_minutes_reserved == 30
    ...
    assert w3.processing_minutes_available == 100
    assert w3.processing_minutes_reserved == 0


def test_reserve_insufficient(session: Session):
    org = new_id("org")
    w = get_or_create_wallet(session, org)
    w.processing_minutes_available = 5
    session.add(w)
    session.commit()
...
def test_debit_reserved(session: Session):
    org = new_id("org")
    w = get_or_create_wallet(session, org)
    w.processing_minutes_available = 50
    session.add(w)
    session.commit()
    reserve_processing_minutes(
        session,
    ...
    session.commit()
    w2 = session.get(CreditWallet, w.id)
    assert w2.processing_minutes_reserved == 8
    assert w2.processing_minutes_spent_lifetime == 12
    assert w2.processing_minutes_available == 30
...
def test_debit_idempotent_no_double_spend(session: Session):
    ...
    session.commit()
    snap = debit_reserved_processing_minutes(
        session,
        organisation_id=org,
        job_id="job_a",
        amount=15,
    ...
    session.commit()
    w2 = session.get(CreditWallet, w.id)
    assert w2.processing_minutes_spent_lifetime == 15
    assert snap.processing_minutes_spent_lifetime == 15
...
def test_completion_debits_and_releases_remainder(session: Session):
    ...
    session.commit()
    w2 = session.get(CreditWallet, w.id)
    assert w2.processing_minutes_reserved == 0
    assert w2.processing_minutes_available == 28
    assert w2.processing_minutes_spent_lifetime == 72


def test_failure_release_idempotent(session: Session):
    org = new_id("org")
    w = get_or_create_wallet(session, org)
    w.processing_minutes_available = 40
    session.add(w)
    session.commit()
    reserve_processing_minutes(
        session,
    ...
    w2 = session.get(CreditWallet, w.id)
    assert w2.processing_minutes_reserved == 0
    assert w2.processing_minutes_available == 40
    ...
    w3 = session.get(CreditWallet, w.id)
    assert w3.processing_minutes_reserved == 0
    assert w3.processing_minutes_available == 40
